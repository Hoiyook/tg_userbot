...
# 包在 import 时会创建 SAVE_FOLDER 并配置日志。
_TMP = tempfile.mkdtemp(prefix="tg_userbot_whitelist_test_")
os.environ["TG_SAVE_FOLDER"] = _TMP

from tg_userbot import whitelist, sources  # noqa: E402
...
class ClassifyMessageChatTest(unittest.TestCase):
    ...
    def test_my_id_returns_me(self):
        result = whitelist.classify_message_chat(
            chat_id=100, my_id=100, whitelist={}
        )
...
class WhitelistPersistenceTest(unittest.TestCase):
    ...
    def test_roundtrip(self):
        ...
        whitelist.save_whitelist(chats, path=self.path)
        self.assertEqual(whitelist.load_whitelist(path=self.path), chats)

    def test_stored_format_is_id_title_pairs(self):
        whitelist.save_whitelist({123: "机器人A"}, path=self.path)
        with open(self.path, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.assertEqual(data, {"chats": [[123, "机器人A"]]})

    def test_missing_file_returns_empty(self):
        self.assertEqual(whitelist.load_whitelist(path=self.path), {})
...
class ParseWlCommandTest(unittest.TestCase):
    ...
    def test_plain_wl_is_list(self):
        self.assertEqual(whitelist.parse_wl_command("/wl"), ("list", None))

    def test_wl_list_is_list(self):
        self.assertEqual(whitelist.parse_wl_command("/wl list"), ("list", None))

    def test_add_with_target(self):
        self.assertEqual(
            whitelist.parse_wl_command("/wl add @DouYintg_bot"),
            ("add", "@DouYintg_bot"),
        )

    def test_add_with_numeric_id(self):
        self.assertEqual(
            whitelist.parse_wl_command("/wl add -100123"), ("add", "-100123")
        )

    def test_add_without_target_returns_empty(self):
        self.assertEqual(whitelist.parse_wl_command("/wl add"), ("add", ""))

    def test_del_with_key(self):
        self.assertEqual(
            whitelist.parse_wl_command("/wl del 123"), ("del", "123")
        )

    def test_del_with_negative_id(self):
        self.assertEqual(
            whitelist.parse_wl_command("/wl del -100123"), ("del", "-100123")
        )

    def test_unknown_subcommand_is_invalid(self):
        self.assertEqual(whitelist.parse_wl_command("/wl foo"), ("invalid", None))

    def test_non_wl_command_returns_none(self):
        self.assertIsNone(whitelist.parse_wl_command("/status"))

    def test_is_wl_command_matches(self):
        for text in ("/wl", "/wl list", "/wl add @x", "/wl del -100123"):
            self.assertTrue(whitelist.is_wl_command(text), text)

    def test_is_wl_command_rejects_others(self):
        for text in ("/done", "/thread 5", "/wadd", "wl"):
            self.assertFalse(whitelist.is_wl_command(text), text)
...
class ResolveWlDelKeyTest(unittest.TestCase):
    ...
    def test_id_match_wins(self):
        self.assertEqual(
            whitelist.resolve_wl_del_key("123", self.whitelist), 123
        )

    def test_negative_id_match(self):
        self.assertEqual(
            whitelist.resolve_wl_del_key("-100456", self.whitelist), -100456
        )

    def test_index_falls_back_to_sorted_order(self):
        # 排序后 [-100456, 123]，序号 2 → 123
        self.assertEqual(
            whitelist.resolve_wl_del_key("2", self.whitelist), 123
        )

    def test_index_out_of_range_returns_none(self):
        self.assertIsNone(whitelist.resolve_wl_del_key("5", self.whitelist))

    def test_non_numeric_returns_none(self):
        self.assertIsNone(whitelist.resolve_wl_del_key("abc", self.whitelist))
...
class ResolveDownloadSourceTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_override_wins_without_forward_lookup(self):
        ...
        with mock.patch.object(
            sources, "get_forward_source",
            side_effect=AssertionError("不该查询转发来源"),
        ):
            source = await sources.resolve_download_source(
                fake_message, "解析机器人"
            )
    ...
    async def test_no_override_falls_back_to_forward_source(self):
        fake_message = mock.Mock()
        with mock.patch.object(
            sources, "get_forward_source", return_value="转发频道"
        ) as m:
            source = await sources.resolve_download_source(fake_message, None)
