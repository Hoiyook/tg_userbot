...
async def handle_command(event, cmd_text):
    ...
    if cmd_text == "/help":
        await _reply(event, 
            ...
            "/chrome <链接> - 智能分流：文件直链自动下载；\n"
            "   网盘页（MEGA/Google盘/OneDrive等）在 Chrome 可见打开\n"
            "/chrome_start | /chrome_stop | /chrome_status - Chrome Agent 启停\n"
            "/paw - Pawchive：扫描作者作品、收藏对比、批量下载\n"
            ...
            "   文件落 原目录/子目录/ 且文件名带 #标注；『/子目录』则只落目录\n"
            "   不加标注；普通评论仍只拼文件名（原语义不变）。\n"
            "🔗 外链备注：发『链接 备注』即登记，重发同链接+新备注可更新，\n"
            "   再次发送会自动查重（含 Pawchive 已完成外链）。\n"
